...
describe("WineQuestionModal.vue", () => {
  ...
  it("emits 'show-past-question' event when clicking on a past question", async () => {
    ...
    const emittedPast = wrapper.emitted("show-past-question");
    expect(emittedPast).toBeTruthy();
    expect(emittedPast && emittedPast[0][0]).toEqual(pastQuestions[0]);
  });

  it("emits 'update:show' event when closeModal is called", async () => {
    await wrapper.find("button[aria-label='Close modal']").trigger("click");

    const emittedUpdateShow = wrapper.emitted("update:show");
    expect(emittedUpdateShow).toBeTruthy();
    expect(emittedUpdateShow && emittedUpdateShow[0][0]).toBe(false);
  });
  ...
  it("emits 'submit-question' with trimmed question text when form is submitted", async () => {
    ...
    // Check the emitted event
    const emittedSubmit = wrapper.emitted("submit-question");
    expect(emittedSubmit).toBeTruthy();
    expect(emittedSubmit && emittedSubmit[0][0]).toBe(expectedQuestion);
  });
  ...
  it("emits 'update:show' event when clicking outside the modal", async () => {
    ...
    const emittedUpdateShow2 = wrapper.emitted("update:show");
    expect(emittedUpdateShow2).toBeTruthy();
    expect(emittedUpdateShow2 && emittedUpdateShow2[0][0]).toBe(false);
  });
