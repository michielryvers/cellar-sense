...
describe("cellar-photo-storage", () => {
  ...
  const mockResizedBlob = new Blob(["resized image data"], {
    type: "image/jpeg",
  });
  ...
  describe("saveCellarPhoto", () => {
    ...
    it("should throw error if database save fails", async () => {
      const { db } = await import("../dexie-db");
      (db.cellarPhotos.add as any).mockRejectedValue(
        new Error("Database error")
      );

      await expect(saveCellarPhoto(mockBlob, [])).rejects.toThrow(
        "Failed to save cellar photo"
      );
    });
  });
  describe("getAllCellarPhotos", () => {
    it("should retrieve all photos sorted by creation date", async () => {
      ...
      (db.cellarPhotos.orderBy as any).mockReturnValue({
        reverse: mockReverse,
      });
    ...
    it("should throw error if database retrieval fails", async () => {
      ...
      await expect(getAllCellarPhotos()).rejects.toThrow(
        "Failed to retrieve cellar photos"
      );
    });
  });
  ...
  describe("getCellarPhoto", () => {
    ...
    it("should throw error if database retrieval fails", async () => {
      const { db } = await import("../dexie-db");
      (db.cellarPhotos.get as any).mockRejectedValue(
        new Error("Database error")
      );

      await expect(getCellarPhoto("photo-1")).rejects.toThrow(
        "Failed to retrieve cellar photo"
      );
    });
  });
  ...
  describe("deleteCellarPhoto", () => {
    ...
    it("should throw error if database deletion fails", async () => {
      const { db } = await import("../dexie-db");
      (db.cellarPhotos.delete as any).mockRejectedValue(
        new Error("Database error")
      );

      await expect(deleteCellarPhoto("photo-1")).rejects.toThrow(
        "Failed to delete cellar photo"
      );
    });
  });
